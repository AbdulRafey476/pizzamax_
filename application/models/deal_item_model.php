<?php
/**
 *Location: application/model/deal_items_model.php
 */
class deal_item_model extends CI_Model {
   ...
   * Collection of Data 
   */
  function COD($array_where = false, $array_like = false) {
        $this->db->select('deal_items.*,foods.*');
        if($array_where != false) {
            $this->db->where($array_where);
        }
        if($array_like != false){
        ...
        $this->db->from('deal_items');
        $this->db->join('foods', 'deal_items.food_id = foods.id', 'right outer'); 
        $query = $this->db->get();
        return $query->result();
  }
  ...
  function insert($data_array) {
    ...
    $this->db->insert('deal_items', $data_array);
  ...
  function update($data_array, $array_where) {
    $this->db->where($array_where);
    $this->db->update('deal_items', $data_array);
  }
}
?>
